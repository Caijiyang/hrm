...
<table width="100%" height="90%" border="0" cellpadding="5" cellspacing="0" class="main_tabbor">
                    ...
                    <td class="fftd">
                        <form action="/document" method="post">
                            ...
                            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td class="font3">
                                        标题：<input type="text" name="title" value='${title==null?"":title}'/>
    ...
    <tr valign="top">
        <td height="20">
            <form id="form1" action="/document" method="post">
                ...
                <table width="100%" border="1" cellpadding="5" cellspacing="0"
                    ...
                    <c:forEach items="${documents}" var="doc">
                        <tr ondblclick="down(7);" class="main_trbg" align="center" id="data_0"
                            ...
                            <td><input type="checkbox" name="ids" id="box_0" value="${doc.id}"></td>
                            <td>${doc.title}</td>
                            <td>${doc.createDate}</td>
                            <td>${doc.user.username}</td>
                            <td>${doc.remark}</td>
                            <td align="center" width="40px;">
                                <a href="/document?operation=update&id=${doc.id}">
                                    <img title="修改" src="../images/update.gif">
                                </a>
                            </td>
                            <td align="center" width="40px;">
                                <a href="${doc.filePath}" id="down_7" download>
                ...
                <tr>
                    <td style="COLOR: #0061de; MARGIN-RIGHT: 3px; PADDING-TOP: 2px; TEXT-DECORATION: none">
                        ...
                        <c:if test='${page.pageNum > 1}'>
                            <a href="/document?operation=selectPage&pageNum=${page.pageNum - 1}&title=${title}">
                                <span>上一页</span>
                            </a>
                        </c:if>
                        ...
                        <c:forEach begin="1" end="${page.totalPage}" var="index">
                            ...
                            <c:if test="${page.pageNum != index}">
                                <a href="/document?operation=selectPage&pageNum=${index}&title=${title}">
                                    <span>${index}</span>
                                </a>
                            </c:if>
                        </c:forEach>
                        ...
                        <form action="/document" method="post" style="display:inline">
                            <input type="hidden" name="operation" value="selectPage">
